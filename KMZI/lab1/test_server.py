import socket
import random
import array

# Константы
HOST = "127.0.0.1"
PORT = 12345
BUFFER_SIZE = 1024

def generate_key():
    return str(random.randint(1000, 9999))

def encrypt_message(message, key):
    encrypted_message = ""
    for char in message:
        encrypted_char = chr(ord(char) + int(key))
        encrypted_message += encrypted_char
    return encrypted_message

def decrypt_message(encrypted_message, key):
    decrypted_message = ""
    for char in encrypted_message:
        decrypted_char = chr(ord(char) - int(key))
        decrypted_message += decrypted_char
    return decrypted_message

#константы   отсюда начинается RC2
RC2_BLOCK_SIZE = 8
MODE_ECB = 0
MODE_CBC = 1
PADDING_PKCS5 = 1

def ROL16 (x, n):
    return ((x << n) | (x >> (16 - n))) & 0xFFFF

def ROR16 (x, n):
    return ((x >> n) | (x << (16 - n))) & 0xFFFF

class RC2():
    def __init__(self, key):
        sbox = array.array('B', [
            ])

        rc2_key = bytearray(128)

        for i in range(128):
            if len(key) > i:
                rc2_key[i] = key[i]
            else:
                rc2_key[i] = sbox[rc2_key[i-1] + rc2_key[i - len(key)] & 0xFF]

        rc2_key[128 - len(key)] = sbox[rc2_key[128 - len(key)]]

        if len(key) < 128:
            for i in range(127 - len(key), -1, -1):
                xor = rc2_key[i + 1] ^ rc2_key[len(key) + i]
                rc2_key[i] = sbox[xor & 0xFF]
        
        self.K = array.array('H')
        self.K.frombytes(rc2_key)

    def decrypt_mixup(self, K, x0, x1, x2, x3, round):

        j = round * 4 + 3
        x3 = ROR16(x3, 5)
        x3 = (x3 - (x1 & x2) - (~x2 & x0)  - K[j]) & 0xFFFF
        j -= 1

        x2 = ROR16(x2, 3)
        x2 = (x2 - (x0 & x1) - (~x1 & x3)  - K[j]) & 0xFFFF
        j -= 1

        x1 = ROR16(x1, 2)
        x1 = (x1 - (x3 & x0) - (~x0 & x2)  - K[j]) & 0xFFFF
        j -= 1

        x0 = ROR16(x0, 1)
        x0 = (x0 - (x2 & x3) - (~x3 & x1)  - K[j]) & 0xFFFF

        return x0, x1, x2, x3
    
    def decrypt_mash(self, K, x0, x1, x2, x3):

        x3 = (x3 - K[x2 & 63]) & 0xFFFF
        x2 = (x2 - K[x1 & 63]) & 0xFFFF
        x1 = (x1 - K[x0 & 63]) & 0xFFFF
        x0 = (x0 - K[x3 & 63]) & 0xFFFF

        return x0, x1, x2, x3
    
    def block_decrypt(self, input_buffer):

        R = array.array('H')
        R.frombytes(input_buffer)

        R[0], R[1], R[2], R[3] = self.decrypt_mixup(self.K, R[0], R[1], R[2], R[3], 15)
        R[0], R[1], R[2], R[3] = self.decrypt_mixup(self.K, R[0], R[1], R[2], R[3], 14)
        R[0], R[1], R[2], R[3] = self.decrypt_mixup(self.K, R[0], R[1], R[2], R[3], 13)
        R[0], R[1], R[2], R[3] = self.decrypt_mixup(self.K, R[0], R[1], R[2], R[3], 12)
        R[0], R[1], R[2], R[3] = self.decrypt_mixup(self.K, R[0], R[1], R[2], R[3], 11)

        R[0], R[1], R[2], R[3] = self.decrypt_mash(self.K, R[0], R[1], R[2], R[3])

        R[0], R[1], R[2], R[3] = self.decrypt_mixup(self.K, R[0], R[1], R[2], R[3], 10)
        R[0], R[1], R[2], R[3] = self.decrypt_mixup(self.K, R[0], R[1], R[2], R[3], 9)
        R[0], R[1], R[2], R[3] = self.decrypt_mixup(self.K, R[0], R[1], R[2], R[3], 8)
        R[0], R[1], R[2], R[3] = self.decrypt_mixup(self.K, R[0], R[1], R[2], R[3], 7)
        R[0], R[1], R[2], R[3] = self.decrypt_mixup(self.K, R[0], R[1], R[2], R[3], 6)
        R[0], R[1], R[2], R[3] = self.decrypt_mixup(self.K, R[0], R[1], R[2], R[3], 5)

        R[0], R[1], R[2], R[3] = self.decrypt_mash(self.K, R[0], R[1], R[2], R[3])

        R[0], R[1], R[2], R[3] = self.decrypt_mixup(self.K, R[0], R[1], R[2], R[3], 4)
        R[0], R[1], R[2], R[3] = self.decrypt_mixup(self.K, R[0], R[1], R[2], R[3], 3)
        R[0], R[1], R[2], R[3] = self.decrypt_mixup(self.K, R[0], R[1], R[2], R[3], 2)
        R[0], R[1], R[2], R[3] = self.decrypt_mixup(self.K, R[0], R[1], R[2], R[3], 1)
        R[0], R[1], R[2], R[3] = self.decrypt_mixup(self.K, R[0], R[1], R[2], R[3], 0)

        return R.tobytes()
    

    def decrypt(self, input_buffer, mode, IV = None, padding = None):

        crypt_size = len(input_buffer)
        crypt_buffer = bytearray(crypt_size)
        
        for i in range(crypt_size):
            crypt_buffer[i] = input_buffer[i]

        for block_counter in range(crypt_size//RC2_BLOCK_SIZE):

            block = crypt_buffer[block_counter * RC2_BLOCK_SIZE:block_counter * RC2_BLOCK_SIZE + RC2_BLOCK_SIZE]

            block_result = self.block_decrypt(block)

            if mode == MODE_CBC:
                if block_counter == 0:
                    if IV is not None:
                        for i in range(RC2_BLOCK_SIZE):
                            crypt_buffer[block_counter * RC2_BLOCK_SIZE + i] = block_result[i] ^ IV[i]
                else:
                    for i in range(RC2_BLOCK_SIZE):
                        crypt_buffer[block_counter * RC2_BLOCK_SIZE + i] = block_result[i] ^ previous_block[i]
            else:
                for i in range(RC2_BLOCK_SIZE):
                    crypt_buffer[block_counter * RC2_BLOCK_SIZE + i] = block_result[i]

            previous_block = block
        
        if padding == PADDING_PKCS5:
            crypt_buffer = crypt_buffer[:-crypt_buffer[crypt_size - 1]]
        return crypt_buffer

#здесь RC2 закончился


def main():# основная функция где вызывается всё
    server_socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
    server_socket.bind((HOST, PORT))
    server_socket.listen(1)
    print("Сервер слушает на порту", PORT)

    try:
        conn, addr = server_socket.accept()
        print("Подключено клиентом:", addr)

        while True:
            choice_button = conn.recv(BUFFER_SIZE).decode()

            if choice_button == "1":
                print("Выбрано действие 1")
                try:
                    secret_key = generate_key()
                    conn.send(secret_key.encode())

                    encrypted_message = conn.recv(BUFFER_SIZE).decode()
                    print("Зашифрованное сообщение от клиента:", encrypted_message)

                    decrypted_message = decrypt_message(encrypted_message, secret_key)
                    print("Расшифрованное сообщение:", decrypted_message)

                except KeyboardInterrupt:
                    print("Сервер завершил работу по запросу пользователя.")
                except Exception as e:
                    print(f"Ошибка при работе с клиентом: {str(e)}")
                finally:
                    pass
                    #conn.close()

            elif choice_button == "2": #кейс предназначенный для RC2
                print("Выбрано действие 2")
                try:
                        # Преобразуйте ключ и байтовый формат, используя ASCII кодировку

                    encrypted_hex,key = (conn.recv(BUFFER_SIZE).decode('utf-8')).split('|')

                    key_bytes = key.encode('ascii')

                    # Создайте объект RC2 с ключом в байтовом формате
                    rc2 = RC2(key_bytes)

                    encrypted_data = bytearray(int(byte, 16) for byte in encrypted_hex.split()) # перенести в decode для превращения в последовательность битов

                    # Расшифруйте данные
                    decrypted_data = rc2.decrypt(encrypted_data, MODE_CBC, bytearray('12345678', 'ascii'), PADDING_PKCS5)

                    # Преобразуйте расшифрованные данные в текстовый формат
                    decrypted_text = decrypted_data.decode('ascii')
                    print('Decrypted Data:', decrypted_text)
                except KeyboardInterrupt:
                    print("Сервер завершил работу по запросу пользователя.")
                except Exception as e:
                    print(f"Ошибка при работе с клиентом: {str(e)}")
                finally:
                    pass
                    #conn.close()

    except KeyboardInterrupt:
        print("Сервер завершил работу по запросу пользователя.")
    except Exception as e:
        print(f"Ошибка при подключении клиента: {str(e)}")
    finally:
        server_socket.close()

if __name__ == "__main__":
    main()
