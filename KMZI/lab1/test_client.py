...
def main():# основная функция где вызывается всё
    ...
    try:
        ...
        while True:
            choice_button = input("Введите 1 или 2 для выбора действия: ")

            if choice_button not in ["1", "2"]:
                print("Неправильный выбор. Введите 1 или 2.")
                continue

            client_socket.send(choice_button.encode())
            ...
            if choice_button == "1":
                try:
                    ...
                    secret_key = client_socket.recv(BUFFER_SIZE).decode()
                    print("Получен секретный ключ от сервера:", secret_key)
                ...
                try:
                        # Введите ваш ключ и данные в текстовом формате
                    key,data = input("Ведите ключ данные которые хотите зашифровать: ").split() 

                        # Преобразуйте ключ и данные в байтовый формат, используя ASCII кодировку
                    ...
                    rc2 = RC2(key_bytes)
                        ...
                        # Зашифруйте данные
                    encrypted_data = rc2.encrypt(data_bytes, MODE_CBC, bytearray('12345678', 'ascii'), PADDING_PKCS5)

                        # Выведите зашифрованную строку в шестнадцатеричном формате
                    encrypted_hex = ' '.join(['{:02X}'.format(byte) for byte in encrypted_data])
                    client_socket.send((encrypted_hex + "|" + key).encode('utf-8'))
