...
class DiffieHellman {
    public:
    void static init_consts() {
        std::cin >> n;
        std::cin >> g;
    }
    
    NTL::ZZ calculation_mod(NTL::ZZ secretNumber) {
        NTL::ZZ mod;
        std::cout << g << "    " << secretNumber << "    " << n << std::endl;
        NTL::PowerMod(mod, g, secretNumber, n);
        return mod;
    }

    NTL::ZZ calculation_key(NTL::ZZ degree, NTL::ZZ secretNumber) {
        NTL::ZZ base = calculation_mod(secretNumber);
        NTL::ZZ key{};
        NTL::PowerMod(key, base, degree, n);
        return key;
    }    
};
...
class RC2 {
    ...
    std::vector<uint64_t> key_extansion(NTL::ZZ key) {
        std::stringstream ss;
        ss << key;
        std::string key_s = ss.str();
...
class User : public DiffieHellman, public RC2 {
    ...
    void gen_number() {
        NTL::RandomLen(this->number, 15);
    }

    void init_secret_number(NTL::ZZ secretNumber) {
        // this->secretNumber = secretNumber;
        NTL::conv(this->secretNumber, secretNumber);
    }
    ...
    void gen_message() {
        ...
        s1 << number;
        s2 << secretNumber;
        // message.resize(s1.str().size() + s2.str().size());
        message = s1.str() + s2.str();
    }
...
int main() {
    ...
    DiffieHellman::init_consts();
    // n = 100007 g = 94735873873
